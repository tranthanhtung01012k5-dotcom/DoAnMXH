    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        commentAdapter = new CommentAdapter(
                ...
                new CommentAdapter.OnCommentActionListener() {
                    @Override
                    public void onCommentClick(CommentModel comment, int position) {}
                    @Override
                    public void onReplyClick(CommentModel comment, int position) {
                        // ✅ LOG kiểm tra toàn bộ thông tin comment
                        Log.d("REPLY_DEBUG", "=== onReplyClick được gọi ===");
                        Log.d("REPLY_DEBUG", "position: " + position);
                        Log.d("REPLY_DEBUG", "comment null? " + (comment == null));

                        if (comment == null) {
                            Log.e("REPLY_DEBUG", "❌ comment là null, return sớm");
                            return;
                        }

                        Log.d("REPLY_DEBUG", "documentId: " + comment.getDocumentId());
                        Log.d("REPLY_DEBUG", "nguoiDungId: " + comment.getNguoiDungId());
                        Log.d("REPLY_DEBUG", "tenDangNhap: " + comment.getTenDangNhap());
                        Log.d("REPLY_DEBUG", "hoVaTen: " + comment.getHoVaTen());
                        Log.d("REPLY_DEBUG", "noiDung: " + comment.getNoiDung());
...
// Mở bàn phím
                        ...
                        // ✅ Nếu tenDangNhap null → thử dùng hoVaTen hoặc load lại từ Firestore
                        if (tenDangNhap == null || tenDangNhap.isEmpty()) {
                            Log.w("REPLY_DEBUG", "⚠️ tenDangNhap null, thử dùng hoVaTen: " + comment.getHoVaTen());

                            // Load lại từ Firestore nếu cả hai đều null
                            if (comment.getNguoiDungId() != null) {
                                db.collection("nguoi_dung").document(comment.getNguoiDungId()).get()
                                        .addOnSuccessListener(userDoc -> {
                                            if (userDoc.exists()) {
                                                Log.d("REPLY_DEBUG", "Firestore trả về ten_dang_nhap: "
                                                        + userDoc.getString("ten_dang_nhap"));
                                                Log.d("REPLY_DEBUG", "Firestore trả về ho_va_ten: "
                                                        + userDoc.getString("ho_va_ten"));

                                                String tenFromDB = userDoc.getString("ten_dang_nhap");
                                                comment.setTenDangNhap(tenFromDB);
                                                // Gọi lại để set mention sau khi có tên
                                                setMentionInEditText(comment);
                                            } else {
                                                Log.e("REPLY_DEBUG", "❌ Không tìm thấy user trong Firestore");
                                            }
                                        })
                                        .addOnFailureListener(e ->
                                                Log.e("REPLY_DEBUG", "❌ Lỗi load user: " + e.getMessage()));
                            }
                            return;
                        }
                    ...
                    private void setMentionInEditText(CommentModel comment) {
                        String mention = "@" + comment.getTenDangNhap();
                        String fullText = mention + " ";
                        Log.d("REPLY_DEBUG", "✅ mention text: " + mention);
                        ...
                        ClickableSpan clickableSpan = new ClickableSpan() {
                            ...
                            public void onClick(@NonNull View widget) {
                                // ✅ LOG kiểm tra uid trước khi mở trang
                                Log.d("MENTION_CLICK", "nguoiDungId: " + comment.getNguoiDungId());
                                Log.d("MENTION_CLICK", "tenDangNhap: " + comment.getTenDangNhap());

                                String uid = comment.getNguoiDungId();
                                if (uid == null || uid.isEmpty()) {
                                    Log.e("MENTION_CLICK", "❌ uid null → không thể mở UserProfile");
                                    Toast.makeText(PostDetailActivity.this, "Không tìm thấy người dùng", Toast.LENGTH_SHORT).show();
                                    return;
                                }

                                Log.d("MENTION_CLICK", "✅ Mở UserProfile với uid: " + uid);
                                Intent intent = new Intent(PostDetailActivity.this, UserProfileActivity.class);
                        ...
                        spannable.setSpan(clickableSpan, 0, mention.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
                                ...
                                public boolean onTouchEvent(TextView widget, Spannable buffer, MotionEvent event) {
                                    if (event.getAction() == MotionEvent.ACTION_UP) {
                                        ...
                                        if (layout != null) {
                                            ...
                                            if (spans.length > 0) {
                                                Log.d("REPLY_DEBUG", "✅ Span click detected tại offset: " + off);
                    ...
                    public void onLikeClick(CommentModel comment, int position) {
                        // Adapter tự xử lý Firestore, không cần làm thêm
                    }
                    ...
                    // ── Callback sau khi sửa bình luận ──────────────────
                    @Override
                    public void onEditComment(CommentModel comment, int position, String newContent) {
                        // Adapter đã update Firestore + notifyItemChanged rồi.
                        // Chỉ cần toast thông báo thêm nếu muốn.
                        Toast.makeText(PostDetailActivity.this,
                                "Đã cập nhật bình luận", Toast.LENGTH_SHORT).show();
                    }

                    // ── Callback sau khi xóa bình luận ──────────────────
                    @Override
                    public void onDeleteComment(CommentModel comment, int position) {
                        // Adapter đã xóa Firestore + remove khỏi list rồi.
                        // Giảm số bình luận trên bài viết:
        ...
        // ── ConcatAdapter — bài viết trên, bình luận dưới ──
    ...
    private void loadLikeFollowAndRepost(PostModel post,
                                         Runnable onDone) {
    ...
    private void checkRepost(PostModel post,
                             Runnable onDone) {
...
//
//                    // Giữ lại avatar/tên của comment cũ
...
//
//                            // Load user info cho comment mới
//                            String uid = comment.getNguoiDungId();
//                            if (uid != null && !uid.isEmpty()) {
...
//
//                                    db.collection("nguoi_dung").document(myUid)
//                                            .collection("nguoi_theo_doi").document(uid).get()
//                                            .addOnSuccessListener(followDoc -> {
...
private void loadComments() {
    db.collection("bai_viet").document(postId)
            ...
            .addSnapshotListener((snapshots, error) -> {
                ...
                // ── Snapshot hiện tại để giữ likedByMe / following ──
                Map<String, CommentModel> oldCommentMap = new HashMap<>();
                for (CommentModel c : commentList) {
                    oldCommentMap.put(c.getDocumentId(), c);
                }
                ...
                for (DocumentSnapshot doc : snapshots.getDocuments()) {
                    ...
                    // ── Giữ lại likedByMe / following từ snapshot cũ ──
                    CommentModel old = oldCommentMap.get(doc.getId());
                    if (old != null) {
                        comment.setLikedByMe(old.isLikedByMe());
                        comment.setFollowing(old.isFollowing());
                    ...
                    // ── Lấy thông tin user từ cache trước, tránh load lại ──
                    String uid = comment.getNguoiDungId();
                    CommentModel cached = uid != null ? userInfoCache.get(uid) : null;

                    if (cached != null) {
                        // Đã có trong cache → dùng luôn, không gọi Firestore
